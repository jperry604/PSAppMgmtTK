...
https://github.com/microsoftgraph/powershell-intune-samples/blob/a9d99f3bed5984dcb8d00fe9aa0af8ed41f1c9ec/LOB_Application/Win32_Application_Add.ps1
...
####################################################
# Creates https://docs.microsoft.com/en-us/graph/api/resources/intune-apps-win32lobapp?view=graph-rest-beta
...
(
	...
	try	{
        ...
        if((($Ext).contains("msi") -or ($Ext).contains("Msi")) -and (!$installCmdLine -or !$uninstallCmdLine)){
            ...
            -uninstallCommandLine $uninstallcmdline -DisplayVersion $Version
...
$SourceFile = Join-path $PSScriptRoot "Intune Package\Deploy-Application.intunewin"
...
#https://docs.microsoft.com/en-us/graph/api/resources/intune-apps-win32lobappregistrydetection?view=graph-rest-beta

$RegistryRule =         $DR = @{ "@odata.type" = "#microsoft.graph.win32LobAppRegistryDetection" }
        $DR.check32BitOn64System = "False";
        $DR.detectionType = "string";
        $DR.detectionValue = "${version}";
        $DR.keyPath = "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{G-U-I-D}";
        $DR.operator = "equal";
        $DR.valueName = "DisplayVersion";

$MSIRule = New-DetectionRule -MSI -MSIproductCode "{G-U-I-D}"

# Creating Array for detection Rule
$DetectionRule = @($FileRule,$RegistryRule,$MSIRule)
#>
$ScriptRule = New-DetectionRule -Powershell -ScriptFile (Join-path $PSScriptRoot "InstallStateDetection.ps1") -enforceSignatureCheck $false -runAs32Bit $false
$DetectionRule = @($ScriptRule)
...
Upload-Win32Lob -SourceFile "$SourceFile" -displayName $displayName -publisher $publisher -description $description `
 -detectionRules $DetectionRule -returnCodes $ReturnCodes `
-installCmdLine "Deploy-Application.exe  -DeploymentType Install -DeployMode Silent" `
-uninstallCmdLine "Deploy-Application.exe  -DeploymentType UnInstall -DeployMode Silent"
